...
function ColorDATA() {
  ...
  let colorCode = hexCode.map(element=>{ 
    return element.code.hex;
  })
 
let size = colorCode.length;  
function generate (){
  let randnum = Math.floor(Math.random()*size); 

let start = randnum;
let end = start + 12
let chosenElements = colorCode.slice(start, end)
setSlicedArray(chosenElements)
// console.log("start is: "+start);
// console.log("end is:"+end);
//console.log("inside: "+chosenElements);
  setColor (colorCode[randnum])
  document.body.style.backgroundColor=colorCode[randnum]
...
useEffect(() => {
  ...
  return (
    <div>
     ...
     <ColorPalette colorFromAPP={slicedArray}/>
